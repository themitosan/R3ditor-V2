		...
		if (fileExists !== false){
			...
			*/
			...
			if (configsList[30] !== undefined){
				SETTINGS_DISABLE_LOG = JSON.parse(configsList[30]);
			} else {
				SETTINGS_DISABLE_LOG = false;
			};
			document.getElementById('R3_SETTINGS_DISABLE_LOG').checked = SETTINGS_DISABLE_LOG;
			...
			// Display position as int instead of hex
			if (configsList[42] !== undefined){
				SETTINGS_DISPLAY_POSITION_INT = JSON.parse(configsList[42]);
			} else {
				SETTINGS_DISPLAY_POSITION_INT = false;
			};
			...
			// (Livestatus) Enable game discovery
			if (configsList[45] !== undefined){
				SETTINGS_LIVESTATUS_ENABLE_GAME_DISCOVER = JSON.parse(configsList[45]);
			} else {
				SETTINGS_LIVESTATUS_ENABLE_GAME_DISCOVER = false;
			};
			document.getElementById('R3_SETTINGS_LIVESTATUS_DISCOVER').checked = SETTINGS_LIVESTATUS_ENABLE_GAME_DISCOVER;
			// Disable eNGE
			if (configsList[46] !== undefined){
				SETTINGS_DISABLE_ENGE = JSON.parse(configsList[46]);
			} else {
				SETTINGS_DISABLE_ENGE = false;
			};
			document.getElementById('R3_SETTINGS_DISABLE_ENGE').checked = SETTINGS_DISABLE_ENGE;
			// Disable RDT Background images
			if (configsList[47] !== undefined){
				SETTINGS_DISABLE_RDT_BACKGROUND = JSON.parse(configsList[47]);
			} else {
				SETTINGS_DISABLE_RDT_BACKGROUND = false;
			};
			document.getElementById('R3_SETTINGS_DISABLE_RDT_BACKGROUND').checked = SETTINGS_DISABLE_RDT_BACKGROUND;
			/*
				End
			*/
...
function R3_SAVE_SETTINGS(reload, logSaving){
	/*
		...
		45: Enable Game Discovery
		46: Disable eNGE
		47: Disable RDT Background Images
	*/
	R3_LOAD_CHECKFILES();
	...
	const newConfigFile = R3_RE3_PATH + '\n' + R3_MERCE_PATH + '\n' + R3_HEX_PATH + '\n' + R3_RE3SLDE_PATH + '\n' + SETTINGS_USE_DISCORD + '\n' + SETTINGS_LIVESTATUS_BAR_POS + '\n' +
						...
						SETTINGS_ENABLE_MOVE_SCREEN_ID + '\n' + SETTINGS_DISPLAY_POSITION_INT + '\n' + R3_RE3MV_PATH + '\n' + R3_RE3PLWE_PATH + '\n' + SETTINGS_LIVESTATUS_ENABLE_GAME_DISCOVER + '\n' + SETTINGS_DISABLE_ENGE + '\n' + SETTINGS_DISABLE_RDT_BACKGROUND;
	try {
		if (R3_WEBMODE === false){
...
function R3_SETTINGS_UPDATE_CHECKBOX(){
	...
	SETTINGS_DISABLE_LOG = JSON.parse(document.getElementById('R3_SETTINGS_DISABLE_LOG').checked);
	SETTINGS_DISABLE_ENGE = JSON.parse(document.getElementById('R3_SETTINGS_DISABLE_ENGE').checked);
	...
	SETTINGS_DISPLAY_POSITION_INT = JSON.parse(document.getElementById('R3_SETTINGS_DISPLAY_POS_INT').checked);
	...
	SETTINGS_DISABLE_RDT_BACKGROUND = JSON.parse(document.getElementById('R3_SETTINGS_DISABLE_RDT_BACKGROUND').checked);
	...
	SETTINGS_LIVESTATUS_ENABLE_GAME_DISCOVER = JSON.parse(document.getElementById('R3_SETTINGS_LIVESTATUS_DISCOVER').checked);
