const cdr = (() => {
  ...
  let sectorData8 = new Int8Array(0),
      sectorData16 = new Int16Array(0),
      sectorData32 = new Int32Array(0);
    ...
    completeCmd: function(self, clock) {
      ...
      switch (currentCommand) {
        ...
        case 0x14:{ let mmss = 0;
                    ...
                    console.debug(`CdlGetTD: ${cdr.params[0]}`, track);
                    cdr.statusCode |= 0x02;
                    cdr.results.push(cdr.statusCode);
    ...
    for (var sg = 0; sg < 18; ++sg) {
      ...
      for (var su = 0; su < 8; ++su) {
        var shiftFilter = sectorData8[sectorOffset + 4 + su],
            shift  = (shiftFilter & 0x0f) >>> 0,
            filter = (shiftFilter & 0xf0) >>> 3,
            k0 = xa2flt[filter + 0],
            k1 = xa2flt[filter + 1];
        ...
        for (var sd = 0; sd < 28; ++sd) {
          ...
          var data = sectorData8[offset] & 0xff,
              index = (shift * 256 + data) * 2,
              s = (sl[1] * k0) + (sl[0] * k1) + xa2pcm[index + (su & 1)];
    ...
    for (var sg = 0; sg < 18; ++sg) {
      ...
      for (var su = 0; su < 8; su += 2) {
        ...
        for (var sd = 0; sd < 28; ++sd) {
          ...
          var data = sectorData8[offset] & 0xff,
              index = (shift * 256 + data) * 2,
              s = (sl[1] * k0) + (sl[0] * k1) + xa2pcm[index + 0];
        ...
        for (var sd = 0; sd < 28; ++sd) {
          ...
          var data = sectorData8[offset] & 0xff,
              index = (shift * 256 + data) * 2,
              s = (sr[1] * k0) + (sr[0] * k1) + xa2pcm[index + 1];
