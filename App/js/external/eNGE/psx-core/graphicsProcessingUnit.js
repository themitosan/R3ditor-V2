...
var packetSizes = [
  ...
  getDisplayArea: function() {
    if ((gpu.status >> 20) & 1) {
      var t = gpu.dispT % 256, b = Math.min(gpu.dispB, 314);
    }
    else {
      var t = gpu.dispT % 240, b = Math.min(gpu.dispB, 263);
    }
    var dispH = b-t,
        dispW = gpu.dispR - gpu.dispL,
        maxwidth = gpu.maxwidths[(gpu.status >> 16) & 7],
        width = dispW / gpu.widths[(gpu.status >> 16) & 7];
    width = Math.min(maxwidth, (width + 2) & ~3);

    var maxheight = gpu.maxheights[(gpu.status >> 19) & 3],
        height = gpu.heights[(gpu.status >> 19) & 3] * dispH;
...
//                console.log(data & 0xf, hex(gpu.result));
  ...
  handlePacketE2: function(data) {
    ...
    var maskx = ((data[0] >>  0) & 0x1f) << 3,
        masky = ((data[0] >>  5) & 0x1f) << 3,
        offsx = ((data[0] >> 10) & 0x1f) << 3,
        offsy = ((data[0] >> 15) & 0x1f) << 3;
